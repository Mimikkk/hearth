...
const names = Object.keys(IconRegistry)
  .slice(0, 10)
  .map(name => ({ name })) as { name: IconName }[];

export const AvailableIconsTab = () => {
  const [queried, query, setQuery] = createQueryable(names, { threshold: 0.4, isCaseSensitive: true, keys: ['name'] });
  ...
  return (
    <div class={s.tab}>
      ...
      <div class="grid grid-cols-8 w-1/2 gap-x-1 gap-y-2 p-2 border" onPointerLeave={() => setTooltip('')}>
        <For each={queried()}>
          {icon => (
            <div class="center w-12 h-12 border rounded-sm bg-primary-2" onPointerEnter={() => setTooltip(icon.name)}>
              <Icon class="w-full" name={icon.name} />
            </div>
          )}
        </For>
      </div>
