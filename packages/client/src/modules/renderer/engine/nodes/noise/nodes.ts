...
export const Noise = {
  fractal: {
    f32: (position = uv(), octaves = 3, lacunarity = 2, diminish = 0.5, amplitude = 1) =>
  ...
  perlin: {
    f32: (position = uv(), amplitude = 1, pivot = 0) => perlin_noise_f32(position).mul(amplitude).add(pivot),
    vec3: (position = uv(), amplitude = 1, pivot = 0) => perlin_noise_vec3(position).mul(amplitude).add(pivot),
  },

  worley: {
    f32: (position = uv(), jitter = 1) => worley_noise_f32(position, jitter, i32(1)),
    vec2: (position = uv(), jitter = 1) => worley_noise_vec2(position, jitter, i32(1)),
    vec3: (position = uv(), jitter = 1) => worley_noise_vec3(position, jitter, i32(1)),
  },
