import { code } from '@modules/renderer/engine/nodes/code/CodeNode.js';
...
`),
  mod_f32: code('fn mod_f32(x: f32, y: f32) -> f32 { return x - y * floor(x / y); }'),
  mod_vec2: code('fn mod_vec2(x: vec2f, y: vec2f) -> vec2f { return x - y * floor(x / y); }'),
  mod_vec3: code('fn mod_vec3(x: vec3f, y: vec3f) -> vec3f { return x - y * floor(x / y); }'),
  mod_vec4: code('fn mod_vec4(x: vec4f, y: vec4f) -> vec4f { return x - y * floor(x / y); }'),
  equals_bool: code('fn equals_bool(a: bool, b: bool) -> bool { return a == b; }'),
  equals_bvec2: code(
