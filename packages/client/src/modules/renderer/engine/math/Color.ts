...
export class Color {
  ...
  static new(r: number = 0, g: number = 0, b: number = 0, a: number = 1): Color {
  ...
  setStyle(style: string, colorSpace: DefinedColorSpace = ColorSpace.SRGB): this {
    function handleAlpha(string: string) {
      ...
      if (parseFloat(string) < 1) console.warn(`engine.Color: Alpha component of ${style} will be ignored.`);
    }
    ...
    if ((m = /^(\w+)\(([^)]*)\)/.exec(style))) {
      ...
      switch (name) {
        ...
        default:
          console.warn('engine.Color: Unknown color model ' + style);
      }
      ...
      if (size === 3) {
        ...
        console.warn('engine.Color: Invalid hex color ' + style);
      }
  ...
  setColorName(style: ColorName, colorSpace: DefinedColorSpace = ColorSpace.SRGB): this {
    ...
    if (hex !== undefined) {
      ...
      // unknown color
      console.warn('engine.Color: Unknown color ' + style);
    }
  ...
  fromVec({ x, y, z }: Vec3): this {
    return this.set(x, y, z);
  }

  applyMatrix3(matrix: Mat3): this {
    const r = this.r,
      g = this.g,
      b = this.b;
  ...
  fromAttribute(attribute: BufferAttribute | InterleavedBufferAttribute, index: number): this {
