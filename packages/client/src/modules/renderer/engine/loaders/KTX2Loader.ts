...
export class KTX2Loader extends Loader {
  ...
  _createTextureFrom(transcodeResult, container) {
    ...
    texture.minFilter = faces[0].mipmaps.length === 1 ? Filter.Linear : Filter.LinearMipmapLinear;
...
async function createRawTexture(container) {
  ...
  for (let levelIndex = 0; levelIndex < container.levels.length; levelIndex++) {
    ...
    if (container.supercompressionScheme === KHR_SUPERCOMPRESSION_NONE) {
      ...
      throw new Error('engine.KTX2Loader: Unsupported supercompressionScheme.');
    }

    let data;

    if (TYPE_MAP[vkFormat] === TextureDataType.Float) {
      data = new Float32Array(
        levelData.buffer,
        levelData.byteOffset,
        levelData.byteLength / Float32Array.BYTES_PER_ELEMENT,
      );
    } else if (TYPE_MAP[vkFormat] === TextureDataType.HalfFloat) {
