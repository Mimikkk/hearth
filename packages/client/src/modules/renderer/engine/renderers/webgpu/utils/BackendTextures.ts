...
export class BackendTextures {
  ...
  getColorBuffer() {
    ...
    const { x: width, y: height } = this.backend.renderer.getDrawSize();
  ...
  getDepthBuffer(depth: boolean = true, stencil: boolean = false) {
    const backend = this.backend;
    const { x: width, y: height } = backend.renderer.getDrawSize();
...
export function getFormat(texture: Texture, device: GPUDevice | null = null) {
  ...
  if (isFramebufferTexture(texture) === true && texture.type === TextureDataType.UnsignedByte) {
    ...
    switch (format as unknown as CompressedTextureFormat) {
      ...
      case CompressedTextureFormat.RGBA_ASTC_12x10:
        formatGPU =
          colorSpace === ColorSpace.SRGB
      ...
      case CompressedTextureFormat.RGBA_ASTC_12x12:
        formatGPU =
          colorSpace === ColorSpace.SRGB
