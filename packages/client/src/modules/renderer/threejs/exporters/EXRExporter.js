...
import { DataUtils, TextureDataType, TextureFormat } from '../Three.js';
import * as fflate from '../../threejs/libs/fflate.module.js';
...
function supportedRTT(renderTarget) {
  ...
  if (renderTarget.texture.type !== TextureDataType.Float && renderTarget.texture.type !== TextureDataType.HalfFloat) {
    throw Error('EXRExporter.parse: Unsupported WebGLRenderTarget texture type.');
  }

  if (renderTarget.texture.format !== TextureFormat.RGBA) {
    throw Error('EXRExporter.parse: Unsupported WebGLRenderTarget texture format, expected TextureFormat.RGBA.');
  }
}
...
function supportedDT(texture) {
  ...
  if (texture.format !== TextureFormat.RGBA) {
    throw Error('EXRExporter.parse: Unsupported DataTexture texture format, expected TextureFormat.RGBA.');
  }
  ...
  if (texture.type === TextureDataType.Float && texture.image.data.constructor.name !== 'Float32Array') {
    throw Error("EXRExporter.parse: DataTexture image data doesn't match type, expected 'Float32Array'.");
  }

  if (texture.type === TextureDataType.HalfFloat && texture.image.data.constructor.name !== 'Uint16Array') {
...
function buildInfoRTT(renderTarget, options = {}) {
  ...
  const WIDTH = renderTarget.width,
    ...
    EXPORTER_TYPE = options.type !== undefined ? options.type : TextureDataType.HalfFloat,
    OUT_TYPE = EXPORTER_TYPE === TextureDataType.Float ? 2 : 1,
...
function getPixelData(renderer, rtt, info) {
  ...
  if (info.type === TextureDataType.Float) {
...
function reorganizeDataBuffer(inBuffer, info) {
  const w = info.width,
    ...
    getValue = info.type == TextureDataType.Float ? getFloat32 : getFloat16,
