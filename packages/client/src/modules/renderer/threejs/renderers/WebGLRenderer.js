...
class WebGLRenderer {
  constructor(parameters = {}) {
    ...
    this._outputColorSpace = ColorSpace.SRGB;
    ...
    this.clear = function (color = true, depth = true, stencil = true) {
      ...
      if (color) {
        ...
        if (_currentRenderTarget !== null) {
          ...
          isIntegerFormat =
            targetFormat === TextureFormat.RGBAInteger ||
            targetFormat === TextureFormat.RGInteger ||
            targetFormat === TextureFormat.RedInteger;
        }
        ...
        if (isIntegerFormat) {
          ...
          const isUnsignedType =
            targetType === TextureDataType.UnsignedByte ||
            targetType === TextureDataType.UnsignedInt ||
            targetType === TextureDataType.UnsignedShort ||
            targetType === TextureDataType.UnsignedInt248 ||
            targetType === TextureDataType.UnsignedShort4444 ||
            targetType === TextureDataType.UnsignedShort5551;
    ...
    function renderTransmissionPass(opaqueObjects, transmissiveObjects, scene, camera) {
      ...
      for (let i = 0, l = transmissiveObjects.length; i < l; i++) {
        ...
        if (material.side === Side.Double && object.layers.test(camera.layers)) {
    ...
    function setProgram(camera, scene, geometry, material, object) {
      ...
      if (refreshMaterial) {
        ...
        materials.refreshMaterialUniforms(
          ...
          _pixelRatio,
          _height,
          currentRenderState.state.transmissionRenderTarget,
        );
      ...
      if (framebuffer) {
        ...
        try {
          ...
          const halfFloatSupportedByExt =
            textureType === TextureDataType.HalfFloat &&
            (extensions.has('EXT_color_buffer_half_float') || extensions.has('EXT_color_buffer_float'));

          if (
            textureType !== TextureDataType.UnsignedByte &&
            utils.convert(textureType) !== _gl.getParameter(_gl.IMPLEMENTATION_COLOR_READ_TYPE) && // Edge and Chrome Mac < 52 (#9513)
            textureType !== TextureDataType.Float &&
            !halfFloatSupportedByExt
          ) {
            console.error(
              'THREE.WebGLRenderer.readRenderTargetPixels: renderTarget is not in TextureDataType.UnsignedByte or implementation defined type.',
            );
            return;
          }
  ...
  set outputColorSpace(colorSpace) {
    ...
    gl.drawingBufferColorSpace = colorSpace === ColorSpace.DisplayP3 ? 'display-p3' : 'srgb';
    gl.unpackColorSpace = ColorManagement.workingColorSpace === ColorSpace.LinearDisplayP3 ? 'display-p3' : 'srgb';
  }
