...
function WebGLShadowMap(_renderer, _objects, _capabilities) {
  ...
  const _shadowMapSize = new Vector2(),
    ...
    _depthMaterial = new MeshDepthMaterial({ depthPacking: DepthPackingStrategy }),
    _distanceMaterial = new MeshDistanceMaterial(),
    _materialCache = {},
    _maxTextureSize = _capabilities.maxTextureSize;

  const shadowSide = { [Side.Front]: Side.Back, [Side.Back]: Side.Front, [Side.Double]: Side.Double };
  ...
  this.render = function (lights, scene, camera) {
    ...
    const toVSM = _previousType !== ShadowMap.VSM && this.type === ShadowMap.VSM;
    const fromVSM = _previousType === ShadowMap.VSM && this.type !== ShadowMap.VSM;
    ...
    for (let i = 0, il = lights.length; i < il; i++) {
      ...
      if (shadow.map === null || toVSM === true || fromVSM === true) {
        const pars = this.type !== ShadowMap.VSM ? { minFilter: Filter.Nearest, magFilter: Filter.Nearest } : {};
      ...
      if (shadow.isPointLightShadow !== true && this.type === ShadowMap.VSM) {
  ...
  function renderObject(object, camera, shadowCamera, light, type) {
    ...
    if (visible && (object.isMesh || object.isLine || object.isPoints)) {
      if (
        (object.castShadow || (object.receiveShadow && type === ShadowMap.VSM)) &&
