...
class MaterialParser {
  ...
  // then switching mat Side.Front -> Side.Back
  // NB: this means that Side.Front and Side.Back have been switched!
  getSide(attributes) {
    if (!attributes.side) return Side.Back;
  ...
  parseTextureNodes(textureNodes) {
    ...
    for (const name in textureNodes) {
      ...
      switch (name) {
        case 'Color':
          maps.map = texture;
          maps.map.colorSpace = ColorSpace.SRGB;
          break;
        case 'Roughness':
        ...
        case 'Specular':
          ...
          maps.specularMap.colorSpace = ColorSpace.SRGB;
          maps.specular = 0xffffff;
          break;
        case 'Luminous':
          maps.emissiveMap = texture;
          maps.emissiveMap.colorSpace = ColorSpace.SRGB;
  ...
  parseEnvMap(connections, maps, attributes) {
    if (connections.envMap) {
      ...
      if (attributes.transparent && attributes.opacity < 0.999) {
        envMap.mapping = Mapping.EquirectangularRefraction;
      ...
      } else envMap.mapping = Mapping.EquirectangularReflection;
